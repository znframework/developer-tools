...
function specialWord($data = '')
{
	...
	$data = preg_replace_callback('/\[((2|3)[0-9]{3}\-[0-9]{2}\-[0-9]{2})\]/', function($change)
	{
		...
		{
			return $dateStyle . ' <span class="label label-danger" style="border:solid; font-weight:100;">new</span>';
		}
	...
	$data = preg_replace('/\$array/i', '$arr', $data);
	$data = preg_replace('/\$object/i', '$obj', $data);
	$data = preg_replace('/\$string/i', '$str', $data);
	...
	$data = preg_replace('/(\s|^|\>|\(|\,)(false|true|null|return)(\s|$|\<|\)|\,)/i', '$1<span style="font-family: consolas,monospace;color:#e62a5a">$2</span>$3', $data);
	$data = preg_replace('/(\s|^|\>|\(|\,)(string|int|\$this|this|array|resource|mixed|bool|object|float|scalar|callable|void)(\s|$|\<|\)|\,)/i', '$1<span style="font-family: consolas,monospace;color:#00bbd0">$2</span>$3', $data);
	$data = preg_replace('/(\s|^|\>)(TINYTEXT|MEDIUMTEXT|LONGTEXT|BIGINT|TINYINT|SMALLINT|MEDIUMINT|UTCDATETIME|CHARACTER\sVARYING|DOUBLE\sPRECISION|USER_DEPRACATED|RECOVERABLE_ERROR|CORE_ERROR|CORE_WARNING|COMPILE_ERROR|COMPILE_WARNING|USER_ERROR|DEPRECATED|USER_WARNING|USER_NOTICE|STRICT|ERROR|WARNING|PARSE|NOTICE|IFNULL|NULLIF|CURRENT_USER|SESSION_USER|SYSTEM_USER|CONNECTION_ID|SCHEMA|SCHEME|LAST_INSERT_UNIQUE|EXISTS|CONSTRAINT|DEFAULT|LIKE|SERIAL|FOREIGN|SEQUENCE|IDENTITY|BETWEEN|LONG|UTC|INTEGER|BINARY|INT|DOUBLE|PRECISION|VARYING|VARCHAR|TIMESTAMP|SMALL|BIG|TINY|MEDIUM|NUMERIC|DECIMAL|FLOAT|TIME|DATETIME|DATE|TEXT|BLOB|CLOB|BEFORE|FOR|ROW|EACH|AFTER|FOLLOWS|PRECEDES|BEGIN|END|IF|CASE|MYSQL|SQL|MYISAM|InnoDB|BDB|CHARACTER|CHAR|COLLATE|REPAIR|OPTIMIZE|GRANT|REVOKE|DESC|ASC|AND|OR|NOT|AUTO|INCREMENT|PRIMARY|KEY|ON|FULL|INSERT|INTO|VALUES|DATABASE|TABLE|ALTER|COLUMN|RENAME|ADD|TO|MODIFY|SET|GET|POST|REQUEST|SESSION|ENV|COOKIE|FILES|SELECT|WHERE|UNION|FROM|HAVING|INNER|JOIN|OUTER|ALL|LEFT|RIGHT|UPDATE|TRIGGER|DELETE|DROP|CREATE|LIMIT|ORDER|BY|GROUP|DISTINCT|SUM|COUNT|MIN|MAX|AVG|ELSE|IN)(\s|$|\<)/', '$1<span style="font-family: consolas,monospace;color:#00bbd0">$2</span>$3', $data);
	$data = preg_replace(['/\{\|/', '/\|\}/'], ['<span style="color:#fc9b9b">{{</span>', '<span style="color:#fc9b9b">}}</span>'], $data);
	...
	$data = preg_replace('/(\s|\>)(znframework)(\s|\<)/i', '$1<span style="color:#2980b9">$2</span>$3', $data);
	$data = preg_replace('/(\s|\>)(package(-\w+){1,}|developer-tools|shared-extension|(single|custom|fullpack|light|shared)\-edition)/i', '$1<span style="color:#2980b9">$2</span>', $data);
